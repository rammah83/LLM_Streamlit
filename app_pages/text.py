...
with st.form(f"{task}"):
    ...
    submit_button = col_submit.form_submit_button("Submit", use_container_width=True)
    if text_length < 2:
        pass
    else:
        col_submit.write("When Ready Click on Submit")
        ...
        if task == "summarization":
            ...
            col_src, col_tgt = col_params.columns(2)
            lang_src = col_src.selectbox(
                "Source Language",
                lang,
                index=0,
            )
            lang_tgt = col_tgt.selectbox(
                "Target Language",
                lang,
                index=0 if len(lang) < 2 else 1,
            )
        if submit_button:
            match task:
                case "text-classification":
                    ...
                    try:
                        ...
                        # display
                        col_label, col_score, col_rest = st.columns(
                            [2, 1, 3], gap="small"
                        )
                ...
                case "summarization":
                    ...
                    outputs = asyncio.run(
                        get_inference(
                            inputs, selected_model, task, parameters=parameters
                        )
                    )
                    st.json(outputs)
                    try:
                ...
                case "question-answering":
                    ...
                    if len(question_text) > 2:
                        ...
                        outputs = asyncio.run(
                            get_inference(inputs, selected_model, task)
                        )
                        # st.json(outputs)
...
st.write(asyncio.run(get_serverless_models_list(task)))
# import torch
